import type { WorkflowItem, WorkflowStats } from "../services/agentApi"
...
export default function WorkflowsPageConnected({ workflows, stats, onCreateWorkflow, onToggleWorkflow, onReplayWorkflow, onDeleteWorkflow }: Props) {
  return (
    <div style={{ padding: "2.5rem 2rem", maxWidth: 860 }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: "2rem" }}>
        ...
        <button className="btn-primary" style={{ borderRadius: 10 }} onClick={onCreateWorkflow}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1rem", marginBottom: "2rem" }}>
        {[
          { label: "Total", value: String(stats.total), icon: "account_tree" },
          { label: "Active", value: String(stats.active), icon: "play_circle", color: "#4ade80" },
          { label: "Running", value: String(stats.running), icon: "sync", color: "#be9dff" },
          { label: "Failed", value: String(stats.failed), icon: "error", color: "#ef4444" },
        ].map(s => (
            ...
            <div key={w.id} className="liquid-glass feature-card" style={{ borderRadius: 14, padding: "1.25rem 1.5rem" }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: "1rem" }}>
                ...
                <div style={{ display: "flex", gap: "0.5rem", flexShrink: 0 }}>
                  <button className="btn-glass" style={{ fontSize: "0.6875rem", padding: "0.3rem 0.75rem", borderRadius: 8 }} onClick={onCreateWorkflow}>
                    <span className="material-symbols-outlined" style={{ fontSize: 13 }}>edit</span>
                  </button>
                  <button className="btn-glass" style={{ fontSize: "0.6875rem", padding: "0.3rem 0.75rem", borderRadius: 8 }} onClick={() => onToggleWorkflow?.(w.id)}>
                    <span className="material-symbols-outlined" style={{ fontSize: 13 }}>{w.status === "paused" ? "play_arrow" : "pause"}</span>
                  </button>
                  <button className="btn-glass" style={{ fontSize: "0.6875rem", padding: "0.3rem 0.75rem", borderRadius: 8 }} onClick={() => onReplayWorkflow?.(w.id)}>
                    <span className="material-symbols-outlined" style={{ fontSize: 13 }}>replay</span>
                  </button>
                  <button className="btn-glass" style={{ fontSize: "0.6875rem", padding: "0.3rem 0.75rem", borderRadius: 8 }} onClick={() => onDeleteWorkflow?.(w.id)}>
